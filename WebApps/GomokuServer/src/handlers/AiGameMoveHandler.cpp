...
void AiGameMoveHandler::handle(const http::HttpRequest &req,
    ...
    try {
        ...
        if (game->isGameOver()) {
            json response = {{"status", "ok"},
                             ...
                             {"next_turn", "none"}};
            std::string responseBody = response.dump();
            resp->setStatusLine(req.getVersion(), http::HttpResponse::k200Ok,
                                "OK");
            resp->setCloseConnection(false);
            resp->setContentType("application/json");
            resp->setContentLength(responseBody.size());
            ...
            {
                std::lock_guard<std::mutex> lock(server_->mutexForAiGames_);
                server_->aiGames_.erase(
                    userId); // 这里删掉之后，每次restart都需要重新创建执行
            }
            return;
        }
        ...
        if (game->isDraw()) {
...

            {
                std::lock_guard<std::mutex> lock(server_->mutexForAiGames_);
                server_->aiGames_.erase(
                    userId); // 这里删掉以后，每次restart都需要重新创建就行
            }
            return;
        }
        ...
        if (game->isGameOver()) {
            json response = {{"status", "ok"},
                             ...
                             {"next_turn", "none"},
                             {"last_move",
                              {{"x", game->getLastMove().first},
                               {"y", game->getLastMove().second}}}};
            std::string responseBody = response.dump();
