...
void LoginHandler::handle(const http::HttpRequest &req,
    ...
    try {
        ...
        if (userId != -1) {
            ...
            if (server_->onlineUsers_.find(userId) ==
                    server_->onlineUsers_.end() ||
                server_->onlineUsers_[userId] == false) {
                {
                    std::lock_guard<std::mutex> lock(
                        server_->mutexForOnlineUsers_);
                ...
                resp->setStatusLine(req.getVersion(),
                                    http::HttpResponse::k403Forbidden,
                                    "Forbidden");
                resp->setCloseConnection(true);
                resp->setContentType("application/json");
...
int LoginHandler::queryUserId(const std::string &username,
