...
class Router {
  ...
  private:
    ...
    void extractPathParameters(const std::smatch &match, HttpRequest &request) {
        // Assuming the first match is the full path, parameters start from
        // index 1
        for (size_t i = 1; i < match.size(); ++i) {
            request.setPathParameters("param" + std::to_string(i),
                                      match[i].str());
        }
    }
  ...
  private:
    ...
    std::unordered_map<RouteKey, HandlerPtr, RouteKeyHash>
        handlers_; // 精准匹配
