...
namespace http {
// 将报文解析出来将关键信息封装到 HttpRequest 对象里面去
        ...
        if (state_ == kExpectRequestLine) {
            ...
            if (crlf) {
                ...
                if (colon < crlf) {
                    ...
                    // 空行，结束 Header
                    // 根据请求方法和 Content-Length 判断是否需要继续读取 body
                    if (request_.method() == HttpRequest::kPost ||
                        ...
                        if (!contentLength.empty()) {
                            ...
                            // POST / PUT 请求没有 Content-Length，是 HTTP
                            // 语法错误
                            ok = false;
                            hasMore = false;
                        }
                ...
                buf->retrieveUntil(crlf + 2); // 开始读指针下一行数据
            } else {
                hasMore = false;
            }
            ...
            // 只读取 Conteng-Length 指定的长度
            std::string body(buf->peek(),
                             buf->peek() + request_.contentLength());
